...
$(function(){
	...
	$("#qd").click(function(){
		...
		var str = "<a href='/WangGe/findGI_user.do?gi="+$select1.val()+"'>"+str2+"</a>" ;
	...
	$("#reset1").click(function(){
		$("#giframe",parent.document.body).attr("src","${pageContext.request.contextPath}/show_SHGoods_gridowner.do");
		return false;
	});
});
	  	...
	  	function  line3(){
			...
			if($("#zdzk").attr("checked")==true){
				if(zkxinxi.length == 0){
					...
					var index_m = 0;
					var index_k = 0;
					$("select[id=select2] option").each(
					  function()
					  {
						  zk = $(this).text();
						  index_m = zk.indexOf("：")+1;
						  index_k = zk.lastIndexOf("]");
						  zk = zk.substring(index_m, index_k);
						  zkgoods_id = zkgoods_id + "," + $(this).attr("value") + ":" + zk;
					  }
					 );
					$("#zkgoods_id").attr("value",zkgoods_id);
					$("#Submit").css("background-color","#D3D3D3");
					$("#Submit").attr("value","创建中...");
