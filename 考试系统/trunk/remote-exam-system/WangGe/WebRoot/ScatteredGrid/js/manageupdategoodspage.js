$(function(){
		$("#rightIframe",parent.document.body).height(700);
	    ...
	    $('form :input').blur(function(){
			 ...
			 if( $(this).is('#hasamount') ){
					if( this.value=="" ){
						...
						var residual_mount2 = $("#residual_mount2").attr("value");
						if(residual_mount2 != ""){
							var residual_mount = $("#residual_mount").attr("value");
							var goods_hasamount2 = $("#goods_hasamount2").attr("value");
							var change = goods_hasamount2-this.value;
							var result = residual_mount2 - change;
							if(result >= 0){
								$("#residual_mount").attr("value",result);
							    var okMsg = '输入正确.';
							    $parent.find(".formtips").remove();
							    $parent.append('<span class="formtips onSuccess">'+okMsg+'</span>');
							}else{
								var errorMsg = '最小值为：'+ (goods_hasamount2 - residual_mount2) + '.';
								$parent.find(".formtips").remove();
		                        $parent.append('<span class="formtips onError">'+errorMsg+'</span>');
		                        return ;
							}
		...
		//重置
		 ...
		 $("#xgtu").click(function(){
				if(this.checked){
					$("#file11").attr("class","required");
					$("#file12").attr("class","required");
					$("#file13").attr("class","required");
					tp.show();
				}else{
					$("#file11").attr("class","");
					$("#file12").attr("class","");
	...
	function goodsnamekeyup(){
	  	...
	  	function goodsinfokeyup(){
	  		var goodsinfo = $("#goodsinfo").attr("value");
	  		if(goodsinfo.length >200){
	  			$("#goodsinfo").val(beforevalue);
