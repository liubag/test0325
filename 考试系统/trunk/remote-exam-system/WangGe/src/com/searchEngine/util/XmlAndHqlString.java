...
public class XmlAndHqlString {
	...
	public static String getEntityNameCount_hql(String keyword_content){
		List<String> keyWord_l=StringUtil.getOption(keyword_content, " ");
		String hql = "select count(*) from "+XmlAndHqlString.entityName+" as e where e."+XmlAndHqlString.rowName+" like '%"+keyWord_l.get(0)+"%'";
		for(int i=1;i<keyWord_l.size();i++){
			hql+="and e."+XmlAndHqlString.rowName+" like '%"+keyWord_l.get(i)+"%'";
		}
	...
	public static String getKeyWord_l_hql(String keyword_content,String otherOrderBy){
		List<String> keyWord_l=StringUtil.getOption(keyword_content, " ");
		String hql = "from "+XmlAndHqlString.entityName+" as e where e."+XmlAndHqlString.rowName+" like '%"+keyWord_l.get(0)+"%'";
		for(int i=1;i<keyWord_l.size();i++){
			hql+="and e."+XmlAndHqlString.rowName+" like '%"+keyWord_l.get(i)+"%'";
		}
	...
	//查询entityName2 ,宝贝数量HQL
	public static String getEntityName2Count_hql(String keyword_content){
		List<String> keyWord_l=StringUtil.getOption(keyword_content, " ");
		String hql = "select count(*) from "+XmlAndHqlString.entityName2+" as e where e."+XmlAndHqlString.rowName2 +" like '%"+keyWord_l.get(0)+"%'";
		for(int i=1;i<keyWord_l.size();i++){
			hql+="and e."+XmlAndHqlString.rowName2+" like '%"+keyWord_l.get(i)+"%'";
		}
		if(XmlAndHqlString.otherWhere != null && XmlAndHqlString.otherWhere.length()>0){
			hql = hql + " and e." + XmlAndHqlString.otherWhere2;
		}
		return hql;
	}
	
	//查询entityName2 ,宝贝集合HQL
	public static String getKeyWord2_l_hql(String keyword_content,String otherOrderBy){
		List<String> keyWord_l=StringUtil.getOption(keyword_content, " ");
		String hql = "from "+XmlAndHqlString.entityName2 + " as e where e."+XmlAndHqlString.rowName2 + " like '%"+keyWord_l.get(0)+"%'";
		for(int i=1;i<keyWord_l.size();i++){
			hql+="and e."+XmlAndHqlString.rowName2 +" like '%"+keyWord_l.get(i)+"%'";
		}
		if(XmlAndHqlString.otherWhere != null && XmlAndHqlString.otherWhere.length()>0){
		...
		if(otherOrderBy != null && otherOrderBy.length() > 0){
			...
			hql = hql + " order by e." + XmlAndHqlString.orderBy2;
		}
		return hql;
	}
