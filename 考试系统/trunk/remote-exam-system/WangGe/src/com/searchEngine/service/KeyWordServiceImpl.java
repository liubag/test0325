...
public class KeyWordServiceImpl extends MyServiceParent implements KeyWordService {
	...
	public void gainGoods_by_keyWord(String keyword_content) {		
		...
		Long number = keyWordGoodsDao.findCount_all(session, XmlAndHqlString.getEntityNameCount_hql(keyword_content));
		number = number + keyWordGoodsDao.findCount_all(session, XmlAndHqlString.getEntityName2Count_hql(keyword_content));
	...
	//分页显示需要的数据
	public List<List<?>> showAll(String keyword_content,int page,int perFolioAmount,String otherOrderBy){
		...
		Query q = session.createQuery(XmlAndHqlString.getKeyWord_l_hql(keyword_content,otherOrderBy));
		if(q.list().size() > 0){
			q.setFirstResult((page-1)*perFolioAmount);
			q.setMaxResults(perFolioAmount);
		}
		l.add((List<?>)q.list());
		q = session.createQuery(XmlAndHqlString.getKeyWord2_l_hql(keyword_content,otherOrderBy));
		if(q.list().size() > 0){
			q.setFirstResult((page-1)*perFolioAmount);
			q.setMaxResults(perFolioAmount);
		}
	...
	public Long findCount(String keywordContent) {
		// TODO Auto-generated method stub
		this.session = this.getSession();
		//判断配置是否为空
		if(XmlAndHqlString.entityName == null){
		...
		Long number = keyWordGoodsDao.findCount_all(session, XmlAndHqlString.getEntityNameCount_hql(keywordContent));
		number = number + keyWordGoodsDao.findCount_all(session, XmlAndHqlString.getEntityName2Count_hql(keywordContent));
