...
public class KeyWordAction extends ActionSupport{
	...
	public String gainGoods_by_keyWord(){
		try {
			...
			keyWordService.gainGoods_by_keyWord(test);			
			return SUCCESS;
		} catch (Exception e) {
			e.printStackTrace();
			return INPUT;
	...
	public String showAll(){
		try {
			...
			this.has = this.keyWordService.findCount(test);
			...
			if(hasPages == 0){
				...
				List<List<?>> l = keyWordService.showAll(test,page, XmlAndHqlString.perFolioAmount,otherOrderBy);
				if(l != null){
					int i = 0;
					for(List ll : l){
						if(i == 0){
